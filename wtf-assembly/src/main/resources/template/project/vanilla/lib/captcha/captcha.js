 ...
 *      &lt;/fieldset&gt;
 *      
 *      &lt;fieldset data-name="captcha" data-class="Captcha"&gt;
 ...
 */
Captcha = class extends HTMLElement {
	/**
	...
	constructor() {
		 ...
		 * @type {Number}
		 */
		this._index = Captcha._INDEX_SEED++;
	 ...
	 */
	reset() {
		this.classList.remove(Captcha._CSS_INVALID);
		[...this.getElementsByTagName("img")].forEach(img => img.classList.remove(Captcha._CSS_SELECTED));
		this._valid = false;
		return this;
	}
	 ...
	 */
	isValid() {
		this.classList.toggle(Captcha._CSS_INVALID, !this._valid);
		return this._valid;
	}
	...
	_onCaptchaImageClick(ev) {
		...
		for (let i = 0; i < images.length; ++i) {
			images.item(i).classList.remove(Captcha._CSS_SELECTED);
		}

		const img = ev.target;
		img.classList.add(Captcha._CSS_SELECTED);
		...
		fetch(Captcha._SERVER_VERIFY_RESPONSE, {
			method: "POST",
	...
	_loadChallenge(ev) {
		...
		fetch(Captcha._SERVER_LOAD_CHALLENGE, {
			method: "POST",
			headers: { "Content-Type": "application/json" },
...
customElements.define("js-captcha", Captcha);
